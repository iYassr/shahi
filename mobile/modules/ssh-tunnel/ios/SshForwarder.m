...
// One session, one socket, many channels — libssh2 multiplexes every channel
// over the session's single TCP socket, so this cannot block on one channel
// without stalling the rest. It runs one non-blocking select() loop over the
// listen socket, every live local socket, and the session socket, shuttling
// bytes between each local connection and its own direct-tcpip channel. That is
// the shape a correct `ssh -L` takes; the alternative (a blocking thread per
// channel) deadlocks the moment two channels are busy, which the sidecar's
// WebSocket-plus-polling guarantees.
//
// VERIFY ON DEVICE: `session.rawSession` and `session.socket` are the two NMSSH
// accessors this leans on. Both exist in current NMSSH; if a linked version
// renames them, this file is the only place to adjust.
...
- (uint16_t)start:(NSError **)error {
  _ssh = session_rawSession(_session);
  _sessionFd = (int)_session.socket;
  if (_ssh == NULL || _sessionFd < 0) {
    if (error) *error = [self errorWithMessage:@"The SSH session did not expose a usable handle."];
    return 0;
  }
...
// NMSSH exposes the raw libssh2 session; isolated here so the one dependency on
// its internals is a single call to adjust if a linked version differs.
static LIBSSH2_SESSION *session_rawSession(NMSSHSession *session) {
  return (LIBSSH2_SESSION *)session.rawSession;
}
