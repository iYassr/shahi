  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("share") { (base64: String, name: String, promise: Promise) in
      guard let data = Data(base64Encoded: base64.padding(toLength: ((base64.count + 3) / 4) * 4, withPad: "=", startingAt: 0)), data.count <= 25 * 1024 * 1024 else {
        promise.reject("invalid_file", "This file could not be saved."); return
      }
      guard let presenter = self.appContext?.utilities?.currentViewController() else {
        promise.reject("no_presenter", "The share sheet is unavailable."); return
      }
      ...
      do {
        ...
        sheet.completionWithItemsHandler = { _, _, _, error in
          ...
          if error != nil { promise.reject("share_failed", "The file could not be shared.") }
          else { promise.resolve(nil) }
        }
        presenter.present(sheet, animated: true)
      } catch {
        try? FileManager.default.removeItem(at: folder)
        promise.reject("save_failed", "The file could not be saved.")
      }
    }.runOnQueue(.main)
  }
}
